...
export default function App() {
  ...
  // NUEVO ESTADO: Controla si se muestra el mini-mapa de las tiendas
  const [showShopsMap, setShowShopsMap] = useState(false);
  ...
  return (
    <div className="touch-pan-y min-h-screen bg-white dark:bg-slate-950 text-slate-800 dark:text-slate-200 font-sans pb-10 transition-colors duration-300">
      ...
      <main className="max-w-md mx-auto p-4 mt-2">
        ...
        {activeTab === 'resumen' && (
          <div className="space-y-4 animate-in fade-in duration-300">
            ...
            {/* SECCIÓN TIENDAS CON MINI-MAPA DESPLEGABLE */}
            <div className="bg-emerald-50 dark:bg-emerald-900/20 rounded-[28px] p-6 border border-emerald-100 dark:border-emerald-900/30">
               <div className="flex items-center justify-between mb-4">
                 <h3 className="font-black text-base text-emerald-900 dark:text-emerald-300 flex items-center gap-2">
                   <ShoppingBag className="w-5 h-5" /> Tiendas Económicas
                 </h3>
                 <button onClick={() => setShowShopsMap(!showShopsMap)} className="text-[10px] font-bold px-3 py-1.5 bg-emerald-200 dark:bg-emerald-800/50 text-emerald-800 dark:text-emerald-300 rounded-full flex items-center gap-1 active:scale-95 transition-all">
                   <MapPin className="w-3 h-3" /> {showShopsMap ? 'Ocultar' : 'Ver Mapa'}
                 </button>
               </div>
               ...
               {/* EL MINI-MAPA QUE SE EXPANDE */}
               {showShopsMap && (
                 <div className="mt-5 rounded-2xl overflow-hidden border border-emerald-200/50 dark:border-emerald-800/50 h-48 relative animate-in slide-in-from-top-2">
                   <iframe 
                      title="Mapa de Tiendas"
                      src="https://maps.google.com/maps?q=Don+Quijote+Matsumoto+Kiyoshi+Tokyo&output=embed"
            ...
            <div className="pt-8 flex justify-end">
              <button 
                ...
                className="p-2 text-slate-300 dark:text-slate-700 hover:text-slate-400 dark:hover:text-slate-500 transition-colors" 
